...
public:
	...
	void findkBiggest(int k, data_type input[m],compressed_data_type *sparse_array,int  *sparse_array_index);
...
void MinHeap::findkBiggest(int k, data_type input[m],compressed_data_type sparse_array[compressed_size],int sparse_array_index[compressed_size]){
	...
	/*	for(int i=0; i<compressed_size; i++){
			for(int l=0; l<8; l++)
			{
				...
				sparse_array[i][(l+1)*j] = arr[l][j];
			}
			std::cout<< sparse_array[0] <<" ";
			}			
		} */
		
//		for(int v=0; v<compressed_size; v++)
//		{
//			for(int m=0; m<num_elements; m++) //words number
//			{			
//				for(int l=total_num_digits-1; l>=0; l--) //bits
//				{
//					sparse_array[v][(total_num_digits*m)+l] = arr[m][l]; 
//				}
//			}
//		}
//		
...
class itemrank{
private:
    int max_iter, index,ind1, ind2, temp;
...
public:
    ...
    void preprocessing_movies (data_type movies_correlation[m][m],compressed_data_type sparse_array[m][compressed_size],int  sparse_array_index[m][compressed_size]){
		...
		for(int i = 0; i < m; i++){
			...
			mheap.findkBiggest(num_elements, movies_correlation[i], sparse_array[i],  sparse_array_index[i]);
			sparse_movies[i] = mheap;
			for(int j=0; j<num_elements; j++)
			{
				std::cout<< "Here "<<sparse_movies[i].getHeap()[j];
			}
		}
	}

	void preprocessing_critics (data_type initial_critics[m][n], int low_index,compressed_data_type sparse_array[compressed_size][n],int  sparse_array_index[compressed_size][n]){
		...
		for(int user = 0; user < 4; user++){
			...
			mheap.findkBiggest(num_elements, critics_col,sparse_array[user], sparse_array_index[user]);
    ...
    void run (data_type movies_correlation[m][m], data_type initial_critics[m][n], int* IR_output,compressed_data_type sparse_movies2[m][compressed_size],compressed_data_type sparse_critics2[compressed_size][n],int sparse_movies_index2[m][compressed_size],int sparse_critics_index2[compressed_size][n]){
		
		//preprocessing for sparcity
		k = num_elements;
		preprocessing_movies(movies_correlation,sparse_movies2,sparse_movies_index2);
        ...
        USER:for(int user = 0; user < n; user = user + 4) { //for each user
         	...
         	preprocessing_critics(initial_critics, user,sparse_critics2,sparse_critics_index2);
            ...
            TIME:for (int t = 0; t < max_iter; t++) { //for each time
                ...
                MOVIE:for(int i = 0; i < m; i = i + 2) { //for each movie
					...
					temp = 0;
					for(int user_var = 0; user_var <= 3; user_var++){
						for(int i_var = 0; i_var < 2; i_var++){
							index = binarySearch(i+i_var,user+user_var);
		                    if(index != -1) IR_col_new[user_var][i+i_var] = a * summation[temp] + (1-a)*sparse_critics[user_var].getHeap()[index];       //rating of i-th movie of user 1
		                    else IR_col_new[user_var][i+i_var] = a * summation[temp];
		                    temp++;
						}
					}
                }
                ...
                UPDATE_IR:for(int j = 0; j < m; j++) {
						...
						IR_col_u3[j] = IR_col_new[2][j];	
						IR_col_u4[j] = IR_col_new[3][j];	
                }
            }
...
//	} 
	...
	*/
	...
	compressed_data_type sparse_movies2[m][compressed_size], sparse_critics2[compressed_size][n];
	int sparse_movies_index2[m][compressed_size], sparse_critics_index2[compressed_size][n];
    int IR_new[n];
    
    for(int i = 0; i < m; i++){
    	for(int j = 0; j < num_elements; j++){
    		sparse_movies2[i][j] = 0;
    		sparse_critics2[i][j] = 0;
    		sparse_movies_index2[i][j] = 0;
    		sparse_critics_index2[i][j] = 0;
		}
	}
    // create object and run Itemrank algorithm
    itemrank obj;
    obj.run(movies_correlation,initial_critics,IR_new,sparse_movies2,sparse_critics2,sparse_movies_index2,sparse_critics_index2);
    // print results
    for(int j = 0; j < n; j++){
        std::cout << IR_new[j] << " ";
    }
